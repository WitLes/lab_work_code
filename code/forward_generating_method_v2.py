...
def topology_generating_type_tree(initial_graph):   # 生成拓扑结构
	try:
		...
		initial_graph.add_weighted_edges_from([[5,9,0],[5,10,0],[5,11,0],[5,12,0],[5,13,0],[5,14,0],[5,15,0]])
		initial_graph.add_weighted_edges_from([[6,9,0],[6,10,0],[6,11,0],[6,12,0],[6,13,0],[6,14,0],[6,15,0]])
		initial_graph.add_weighted_edges_from([[7,9,0],[7,10,0],[7,11,0],[7,12,0],[7,13,0],[7,14,0],[7,15,0]])
		initial_graph.add_weighted_edges_from([[8,9,0],[8,10,0],[8,11,0],[8,12,0],[8,13,0],[8,14,0],[8,15,0]])
...
def topology_generator(type=0):  # 生成不同的网络模型
	...
	if type == 1:
		...
		return GNP
	if type == 2:
		ER = nx.erdos_renyi_graph(30,0.2)
		return ER
...
def directed_and_weighted_graph_generator(graph,sample_graph):    #将之前生成的网络模型转化成有向有权图
...
G = nx.DiGraph() # 初始化
G = directed_and_weighted_graph_generator(G,sample_graph)  # 将样本重构成有向有权图
draw_graph(G,type=0)
